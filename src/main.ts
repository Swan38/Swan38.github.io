...
import { Participant, History, Group, Next, RawData as ComponentRawData } from "./components"
...
namespace RawData { // Raw data storage
    ...
    participant.addEventListener('update', () => { debounce_store_raw_data() })
    history.addEventListener('update', () => { debounce_store_raw_data(0) })
    group.addEventListener('update', () => { debounce_store_raw_data(0) })
